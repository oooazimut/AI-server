...
def build_task_create_draft(
    ...
    *,
    ...
    responsible_id, responsible_note = _extract_responsible_id(cleaned, task)
    group_id, group_note = _extract_group_id(cleaned)
    deadline, deadline_note, no_deadline = _extract_deadline(cleaned, now=now)
    title = _extract_title(cleaned)

    missing_fields: list[str] = []
    notes = [note for note in (responsible_note, group_note, deadline_note) if note]
    fields: dict[str, Any] = {}
...
def format_task_create_clarification(draft: BitrixTaskCreateDraft) -> str:
    ...
    parts.append("Для первого MVP укажи ответственного явно: `на меня` или `ответственный #123`.")
    if draft.notes:
...
def _extract_responsible_id(text: str, task: AgentTask) -> tuple[int | None, str]:
    normalized = _normalize(text)
...
def _extract_group_id(text: str) -> tuple[int | None, str]:
    match = re.search(
    ...
    if re.search(r"(?:в\s+)?(?:проекте|проект|группе|группа)\s+[а-яa-z0-9_-]", text, flags=re.IGNORECASE):
        return None, "Проект указан названием; в MVP нужен явный `проект #ID` или отдельный resolver проекта."
    return None, ""
