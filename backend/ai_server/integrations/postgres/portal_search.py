...
from ai_server.integrations.bitrix.portal_search.search_index import (
    ...
    to_str,  # noqa: F401 — re-used for type parity
)
...
class PostgresPortalSearchIndex:
    """Drop-in PG replacement for PortalSearchIndex, storing items in bitrix24.portal_search_items."""
    ...
    async def ensure_schema(self) -> None:
        async with await psycopg.AsyncConnection.connect(self._url, row_factory=dict_row) as db:
            await db.execute(sql.SQL("CREATE SCHEMA IF NOT EXISTS {}").format(sql.Identifier(_SCHEMA)))
            await db.execute(
                f"""
            ...
            await db.execute(f"CREATE INDEX IF NOT EXISTS idx_psi_type    ON {_TABLE}(entity_type)")
            await db.execute(f"CREATE INDEX IF NOT EXISTS idx_psi_indexed ON {_TABLE}(indexed_at)")
            await db.execute(f"CREATE INDEX IF NOT EXISTS idx_psi_seen    ON {_TABLE}(last_seen_at)")
        ...
        try:
            with self._connect() as db:
                row = db.execute(
                    ...
                    "WHERE table_schema = %s AND table_name = 'portal_search_items'",
                    (_SCHEMA,),
                ).fetchone()
            return bool(row)
        except Exception:
    ...
    def content_readiness(self, *, allowed_extensions: set[str]) -> PortalContentReadiness:
        ...
        with self._connect() as db:
            rows = db.execute(
                f"""
                SELECT title, metadata_json FROM {_TABLE}
                WHERE entity_type IN ('disk_file', 'task_attachment')
                """
            ).fetchall()
