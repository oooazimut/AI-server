...
async def _handle_quality_control_webhook_task(
    ...
    *,
    ...
    if not task_detail:
        return {"handled": False, "reason": "task_detail_not_found", "event": event_type, "task_id": task_id}

    task_status = _to_str(_first(task_detail, "status", "STATUS"))
    closed_by = _to_int(_first(task_detail, "closedBy", "CLOSED_BY"))
    should_check_waiting_control = task_status == "4"
    should_check_external_close = (
        task_status == "5"
        and closed_by is not None
        and closed_by not in _trusted_quality_closer_ids()
    )
    if not should_check_waiting_control and not should_check_external_close:
        return {
            "handled": False,
            "reason": "status_not_waiting_control",
            "event": event_type,
            "task_id": task_id,
    ...
    if is_quality_exempt_responsible(responsible_id):
        approved = False
        if task_status == "4" and not settings.quality_control_dry_run:
            await bitrix.approve_task(task_id)
            approved = True
        return {
            "handled": True,
            "reason": "exempt_responsible_auto_approved",
            "event": event_type,
            "task_id": task_id,
    ...
    process_key = _webhook_quality_process_key(task_id, task_detail, result, result_text)
    ...
    try:
        task = await _build_quality_task_from_snapshot(task_detail, raw_results, reviewer=reviewer)
    ...
    ledger[process_key] = {
        ...
        "valid": bool(task and not task.is_invalid),
