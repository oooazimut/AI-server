...
Channel Adapter: Bitrix24 chat / local test
  ↓
Переговорщик / внутренний оркестратор
  ↓
...
`internal_orchestrator` - технический id роли, которую в обсуждении называем
`Переговорщик`: он принимает сообщения людей, выбирает специалистов и является
голосом системы во внешнем чате. Субагенты-специалисты не должны сами отвечать в
чат как отдельные собеседники; они возвращают результат Переговорщику или
канальному runtime.
...
Для Bitrix24 это означает: `Bitrix24Specialist` исполняет готовые
структурированные команды оркестратора, а техническая индексация, reconciler и
очередь webhook-событий живут в слое `Workers / Automations` и не принимают
бизнес-решений.

## Почему пока без прямой зависимости от LangGraph/MCP

Смысловые skills, шаблоны и правила маршрутизации принадлежат оркестратору.
Bitrix публикует возможности, проверяет контракт, права и OAuth, затем вызывает
точно выбранный инструмент. После стабилизации сценариев можно заменить
внутренний runtime на LangGraph и вынести Bitrix/tools в MCP-серверы без
изменения этой границы ответственности.
