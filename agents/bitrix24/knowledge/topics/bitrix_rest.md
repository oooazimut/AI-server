...
`tasks.task.result.add` - добавить результат выполнения задачи.

`tasks.task.complete` - перевести задачу в выполненные/на контроль. Если `TASK_CONTROL=Y`, задача перейдёт в STATUS=4 (ждёт контроля).
...
- если пользователь не указал срок, ставит дедлайн по умолчанию: три рабочих дня от даты создания, 19:00 МСК;
...
`sonet_workgroup.list` НЕ существует в Bitrix24 REST API (вернёт `ERROR_METHOD_NOT_FOUND`). Для списка
и поиска проектов используется только `sonet_group.get`.

`sonet_group.get` без `params` или с пустым `FILTER` - список всех рабочих групп / проектов:
...
`sonet_group.get` с `FILTER: {"ID": 42}` - получить конкретную группу/проект по ID:
...
Поиск проекта по имени: `FILTER` по `NAME`/`%NAME` в `sonet_group.get` на этом портале не работает
(Bitrix молча игнорирует фильтр и возвращает все проекты). Поэтому для поиска по названию вызывай
`sonet_group.get` без фильтра (получишь все проекты, их немного) и сам сопоставляй название в ответе.
...
`im.notify.system.add` — отправить системное уведомление конкретному пользователю. Выполняется немедленно (не требует подтверждения).
...
```json
{
  ...
  "params": {
    "USER_ID": 9,
    "MESSAGE": "Срок задачи #101 «Проверить камеру» истёк вчера."
  },
  "summary": "Уведомить пользователя #9 о просроченной задаче"
}
```

Для уведомления нескольких пользователей вызывать `im.notify.system.add` несколько раз (по одному для каждого `USER_ID`).
