...
- если `DEADLINE` не передан, добавляет дедлайн по умолчанию;
- если включён `AGENT_CREATED_TASKS_REQUIRE_RESULT`, после `tasks.task.add` дополнительно включает `requireResult=true` через Bitrix REST v3;
- если включён `AGENT_CREATED_TASKS_TASK_CONTROL`, включает контроль результата.
...
Для физического чтения/сравнения/создания Word-документа использовать не прямой `bitrix_api`, а `document_access`.
