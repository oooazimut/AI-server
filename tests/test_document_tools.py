...
def test_document_toolset_compares_spreadsheets(monkeypatch, tmp_path):
    ...
    preview_tool = SpreadsheetPreviewTool(client, portal_search=index, settings=settings)
    compare_tool = SpreadsheetCompareTool(client, portal_search=index, settings=settings)

    preview = anyio_run(preview_tool.execute({"query": "смета январь"}, user_id=9))
...
def test_spreadsheet_compare_requires_llm_selected_schema(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("AI_SERVER_VAR_DIR", str(tmp_path / "var"))
    index = _document_index(tmp_path / "search_index.sqlite")
    compare_tool = SpreadsheetCompareTool(FakeDocumentBitrix(), portal_search=index, settings=get_settings())
...
def test_pto_specialist_uses_spreadsheet_compare_tool(monkeypatch, tmp_path):
    ...
    specialist = PtoSpecialist(
        ...
        agent_tools=[
            SpreadsheetPreviewTool(client, portal_search=index, settings=settings),
            SpreadsheetCompareTool(client, portal_search=index, settings=settings),
            DocumentDraftCreateTool(settings),
            DocumentDraftListTool(settings),
            DocumentSearchTool(portal_search=index, settings=settings),
            DocumentReadTool(client, portal_search=index, settings=settings),
        ],
        llm=fake_llm,
    )
    ...
    preview_actions = [item for item in result.actions_taken if item.name == "spreadsheet_preview"]
    assert len(preview_actions) == 2
    action = next(item for item in result.actions_taken if item.name == "spreadsheet_compare")
