...
def test_prompt_contract_contains_context_catalog_and_hard_constraints():
    prompt = planner_prompt(plan_id="p", request="Только Bitrix: найди склад", dialog_history=[{"role": "user", "content": "before"}])
...
def test_extra_model_branch_not_requested_by_text_is_rejected_before_dispatch():
    ...
    raw = json.dumps({"schema_version": PLAN_SCHEMA, "plan_id": "p", "request_hash": sha256_text(request), "state": "EXECUTE", "clarification": None, "max_rounds": 1, "subtasks": [{"subtask_id": "extra", "capability": "delivery", "input": {"query": "Борисов"}}]}, ensure_ascii=False)
...
def test_boolean_round_limit_and_free_text_final_are_rejected_before_publish():
    request = "Найди склад Борисова"
    boolean_rounds = json.dumps({"schema_version": PLAN_SCHEMA, "plan_id": "p", "request_hash": sha256_text(request), "state": "EXECUTE", "clarification": None, "max_rounds": True, "subtasks": [{"subtask_id": "lookup", "capability": "warehouse_lookup", "input": {"query": "Борисов"}}]}, ensure_ascii=False)
    ...
    ungrounded_final = json.dumps({"schema_version": "t0006.final.v1", "plan_id": "p", "response_hash": sha256_text(raw), "answer": "Выдуманный ответ", "ordered_subtask_ids": ["lookup"]}, ensure_ascii=False)
...
def test_general_possible_typo_and_ambiguous_label_require_zero_call_clarification():
    for request in ("Найди склад Карисова", "Покажи гараж"):
        raw = json.dumps({"schema_version": PLAN_SCHEMA, "plan_id": "p", "request_hash": sha256_text(request), "state": "EXECUTE", "clarification": None, "max_rounds": 1, "subtasks": [{"subtask_id": "lookup", "capability": "warehouse_lookup", "input": {"query": request}}]}, ensure_ascii=False)
