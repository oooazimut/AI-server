...
Существующие порты:
- `agents/ports.py` — `SchedulerPort`, `BitrixToolsetPort`, `PtoToolsetPort`, `VehicleUsageToolsetPort`
...
`_build_orchestrator()`, `BitrixWebhookProcessor`, `build_specialist_registry()` — не трогать.
...
**Оркестратор** (`internal_orchestrator`) получает запрос, привлекает нужных специалистов, синтезирует ответ.

**Специалисты:**
| ID | Класс | Область |
|---|---|---|
| `bitrix24` | `Bitrix24Specialist` | Задачи, диск, поиск по порталу, документы |
...
│   ├── ports.py                    # SchedulerPort, BitrixToolsetPort, PtoToolsetPort, VehicleUsageToolsetPort
...
│
├── orchestrators/
│   ├── internal.py                 # InternalOrchestrator (синглтон)
│   └── internal_llm.py             # InternalLLMRouter, InternalOrchestratorLLM Protocol
│
├── channels/
...
`build_specialist_registry`, `_build_orchestrator`, `BitrixWebhookProcessor` — не трогать.
